...
func GetTokenPriceTS(c *gin.Context) {
	...
	if res := configs.MongoPriceCol.FindOne(c, bson.M{"token_id": tokenId, "level": level}); res.Err() != nil {
		c.IndentedJSON(http.StatusInternalServerError, map[string]interface{}{"err": res.Err().Error()})
	} else {
		tp := schema.TokenPrice{}
		err := res.Decode(&tp)
		if convRial {
			if err := convTSCurrency(c, &tp, schema.Currency(currency), tp.Level); err != nil {
				c.IndentedJSON(http.StatusInternalServerError, map[string]interface{}{"err": err.Error()})
				return
			}
		}
		if err != nil {
...
func GetTokenPriceTSFlat(c *gin.Context) {
	tokenId := c.Query("tokenId")
	if len(tokenId) == 0 {
		c.IndentedJSON(http.StatusUnprocessableEntity, -1)
		return
	...
	if res := configs.MongoPriceCol.FindOne(c, bson.M{"token_id": tokenId, "level": level}); res.Err() != nil {
		...
		// ts := make([]schema.TimeSeriesFlat,0)
		// resp := schema.TokenPriceFlat{
		// 	Detail: tp.Detail,
		// 	Level: tp.Level,
		// 	TokenId: tp.TokenId,
