...
var (
	CGPlatformChainMap = make(map[int64]int64)
	CGPlatforms        schema.Platforms
)

func LoadCG() {
	var byteValue []byte
	if _, err := os.Stat(Config.CGPlatformsDir); errors.Is(err, os.ErrNotExist) {
		resp, err := http.Get(Config.CGPlatformsURL.String())
		if err != nil {
			Logger.Panicln(err)
		}
